<script setup>
  import { ref, onMounted } from 'vue'         //ref is used to directly access an underlying DOM element
  //Registers a callback to be called after the component has been mounted.
  import { RouterLink, RouterView } from 'vue-router'  //instead of using regular a tags, we use a custom component router-link to create links. 
  //router-view component is used to display the component that corresponds to the current route.
  import MenuItem from './components/MenuItem.vue';
  import MusicPlayer from './components/MusicPlayer.vue'
  import ChevronUp from 'vue-material-design-icons/ChevronUp.vue';         //chevron is an icon used to slide song list or toggle a dropdown menu
...
<template>
    <div>
        <div 
          ...
          "
        >
        <!-- //Tailwind flex is a class that specifies how much the item will grow relative to the rest of the flexible items. -->
        <!-- items-center to align items along the center of the container -->
        <!-- justify-between:The flex elements are spaced evenly, with the first item pushed to the left and the last item pushed to the right. -->
 ...
 <!-- if open menu is !=to openmenu then onlick open menu and set bg to grey else if openmenu ==openmenu then onlick close openmenu and set bg to black -->
           <button @click="openMenu = !openMenu" :class="openMenu ? 'bg-[#282828]' : 'bg-black'"    
                ...
                <div class="flex items-center">
                    <img 
                      class="rounded-full" 
                      width="27"
                      src="https://www.nicepng.com/png/detail/367-3671905_person-icon-person-icon-silhouette.png"
                    >
                   <div class="text-white text-[14px] ml-1.5 font-semibold">Priyanka SP</div>    <!-- v-if is used to conditionally render a block -->
                     <ChevronDown v-if="!openMenu" @click="openMenu = true" fillColor="#FFFFFF" :size="25" /> <!--v-else is used to toggle the element if the IF condition is not satisfied -->
